...
from lesson5.config import CLIENT_DATABASE, COMMON_CHAT, COMMON_CHAT_PWD
...
class ClientStorage:
    ...
    def add_common_chat_when_creating_db(self):
        if not self.session.query(self.ContactList).filter_by(user_login=COMMON_CHAT).count():
            self.user_registration(COMMON_CHAT, COMMON_CHAT_PWD)
            self.session.commit()

    def user_registration(self, name, password):

        check_user = self.session.query(self.ContactList).filter_by(user_login=name)
        ...
        # если нет, добавляем его в список пользователей
            ...
            user = self.ContactList(name, datetime.now(), sha_pass_str)
    ...
    def get_all_users(self):
        ...
        return [user[0] for user in self.session.query(self.ContactList.user_login).all()]

    def add_contact(self, user, contact):
        # Получаем ID пользователей
        user = self.session.query(self.ContactList).filter_by(user_login=user).first()
        contact = self.session.query(self.ContactList).filter_by(user_login=contact).first()
    ...
    def get_contacts(self, username):
        q = self.session.query(self.ContactList).filter_by(user_login=username)
        if q.count() != 0:
            user = q.one()
            contacts = self.session.query(self.UsersContacts, self.ContactList.username). \
                filter_by(username=user.id).join(self.ContactList, self.UsersContacts.contact == self.ContactList.id)
